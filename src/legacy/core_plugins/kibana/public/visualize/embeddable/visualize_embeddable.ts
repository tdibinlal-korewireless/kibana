...
import { APPLY_FILTER_TRIGGER, Embeddable, getEvent, getTriggerActions } from 'ui/embeddable';
...
export interface VisualizeOutput {
  ...
  query?: Query;
  filters?: Filters;
  usingAdvancedExpression: boolean;
}
...
export class VisualizeEmbeddable extends Embeddable<VisualizeInput, VisualizeOutput> {
  ...
  private filterListener = async (filters: Filter[]) => {
    ...
    const events = await getTriggerActions(APPLY_FILTER_TRIGGER);
    events.execute({
      embeddable: this,
