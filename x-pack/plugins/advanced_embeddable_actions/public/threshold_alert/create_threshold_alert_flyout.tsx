...
export class CustomizeTimeRangeFlyout extends Component<CustomizeTimeRangeProps, State> {
  ...
  private save = () => {
    createThresholdAlert({ serverSideActionId: 'sendToSlack' | 'sendEmail' | 'createAndAddVisualizationToAlertsDashboard' });
  };

  // SendToSlackUIAction
  import { SendToSlackAPI } from 'alert-actions';


  
  public execute({ embeddable }) {
    SendToSlackAPI.execute({ title: embeddable.title, channel: })
  }
